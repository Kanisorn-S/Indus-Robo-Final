...
class URARM:
    ...
    ROTATE_TCP_SLEEP = 1
    MOVEL_SLEEP = 1.5
    TOTAL_SLEEP = ROTATE_TCP_SLEEP + (3 * MOVEL_SLEEP)
    # HOME_X = 0.116
    # HOME_Y = -0.300
    # HOME_Z = 0.2
    ...
    def grab_after_t(self, x_rel: float, y_rel: float, rz: float, t1: float, t2: float, t3: float, t4: float):
        total_time = t1 + t2 + t3 + t4 + self.TOTAL_SLEEP
        x_m = x_rel - (0.02 * total_time)
        self.rotate_TCP(rz=rz, t=t1)
        self.movel(URARM.relative_pose(z=-0.1, t=t2))
        self.movel(URARM.relative_pose(x=x_m, y=y_rel, t=t3))
        self.movel(URARM.relative_pose(z=-0.16, t=t4))
